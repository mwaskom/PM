function PM_MakeRegsMnemonic(par)
...
for l=1:length(class_set)
    ...
    %for p=1:length(perf_set)
        ...
        for c = 1:length(conf_set)
            idx.thisOns =  idx.(class_set{l}) .* idx.C{c};
            %idx.thisOns = idx.(perf_set{p}) .* idx.(class_set{l}) .* idx.C{c};
            ...
            if ~isempty(idx.alltrials(find(idx.thisOns)))
                ...
                i = i+1;
                fName = sprintf('%s_conf%g', class_set{l},  c);
                %fName = sprintf('%s_%s_conf%g', class_set{l},  perf_set{p},  c);
