function PM_setcontrasts_analysisRetByCohAndConf(subpar)
...
idx.coh = ~cellfun('isempty', strfind(regNames,'coh'));
idx.conf = ~cellfun('isempty', strfind(regNames,'conf'));

idx.bf1 = ~cellfun('isempty', strfind(regNames,'bf(1)'));

%
%con.greaterCohCor = idx.coh .* idx.cor;
%con.greaterCohCor_x_face_vs_house = idx.coh .* idx.cor .* (idx.face - idx.house);
con.greaterConfCor_x_face_vs_house = idx.conf .* idx.cor .* (idx.face - idx.house);

con.greaterConfInc = idx.conf .* idx.inc ;
%con.greaterCohInc = idx.coh .* idx.inc;

%con.greaterCohAll = idx.coh;
con.greaterConfAll = idx.conf;
...
con.greaterConfCor = idx.conf .* idx.cor;
