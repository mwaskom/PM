function PM_setcontrasts_Quartiles(subpar)
...
idx.CohQ1 = ~cellfun('isempty', strfind(regNames,'Q1'));
idx.CohQ2 = ~cellfun('isempty', strfind(regNames,'Q2'));
idx.CohQ3 = ~cellfun('isempty', strfind(regNames,'Q3'));
idx.CohQ4 = ~cellfun('isempty', strfind(regNames,'Q4'));
...
% %
% idx.trial.color = idx.istrial .* idx.bf1 .* idx.color;
% idx.trial.shape = idx.istrial .* idx.bf1 .* idx.shape;
% idx.trial.pattern = idx.istrial .* idx.bf1 .* idx.pattern;
% 
% idx.trial.same = idx.istrial .* idx.bf1 .* idx.same;
% idx.trial.diff = idx.istrial .* idx.bf1 .* idx.diff;
% 
% idx.trial.match = idx.istrial .* idx.bf1 .* idx.match;
% idx.trial.nonmatch = idx.istrial .* idx.bf1 .* idx.nonmatch;
% 
% %
% idx.cue.color = idx.iscue .* idx.bf1 .* idx.color;
% idx.cue.shape = idx.iscue .* idx.bf1 .* idx.shape;
% idx.cue.pattern = idx.iscue .* idx.bf1 .* idx.pattern;
% 
% idx.cue.same = idx.iscue .* idx.bf1 .* idx.same;
% idx.cue.diff = idx.iscue .* idx.bf1 .* idx.diff;


% vis_features = {'color' 'shape' 'pattern'};
% same_diff = {'same' 'diff'};
%
% con.trial_color_vs_shape_and_pattern = 2*idx.trial.color - idx.trial.shape - idx.trial.pattern;
% con.trial_shape_vs_color_and_pattern = 2*idx.trial.shape - idx.trial.color - idx.trial.pattern;
% con.trial_pattern_vs_color_and_shape = 2*idx.trial.pattern - idx.trial.color - idx.trial.shape;
% 
% con.trial_same_vs_diff = idx.trial.same - idx.trial.diff;
% con.trial_match_vs_non = idx.trial.match - idx.trial.nonmatch;

% fn_trial = fieldnames(idx.trial);
%for f = 1:length(fn_trial)
%    thisConName = ['trial_' fn_trial{f} '_vs_fix'];
%    con.(thisConName) = idx.trial.(fn_trial{f});
%end

% con.cue_color_vs_shape_and_pattern = 2*idx.cue.color - idx.cue.shape - idx.cue.pattern;
% con.cue_shape_vs_color_and_pattern = 2*idx.cue.shape - idx.cue.color - idx.cue.pattern;
% con.cue_pattern_vs_color_and_shape = 2*idx.cue.pattern - idx.cue.color - idx.cue.shape;
% 
% con.cue_same_vs_diff = idx.cue.same - idx.cue.diff;
%     
...
con.binnedHalvesCoh_corOnly = ((idx.CohQ3 + idx.CohQ4) - (idx.CohQ1 + idx.CohQ2) ).* idx.cor .* idx.bf1 ;
con.LinearCoh_corOnly = (1.5*idx.CohQ4 + .5*idx.CohQ3 + -.5*idx.CohQ2 + -1.5*idx.CohQ1).* idx.cor  .* idx.bf1 ;
con.LinearCoh_Face_corOnly = (1.5*idx.CohQ4 + .5*idx.CohQ3 + -.5*idx.CohQ2 + -1.5*idx.CohQ1).* idx.face .* idx.cor  .* idx.bf1 ;
con.LinearCoh_House_corOnly = (1.5*idx.CohQ4 + .5*idx.CohQ3 + -.5*idx.CohQ2 + -1.5*idx.CohQ1).* idx.house .* idx.cor  .* idx.bf1 ;


con.Q4VsQ1_corOnly = (idx.CohQ4 - idx.CohQ1) .* idx.cor  .* idx.bf1 ;
con.Q4VsQ1_Face_corOnly = (idx.CohQ4 - idx.CohQ1) .* idx.cor  .* idx.face .* idx.bf1 ;
con.Q4VsQ1_House_corOnly = (idx.CohQ4 - idx.CohQ1) .* idx.cor  .* idx.house .* idx.bf1 ;
