function [zscoreA_cell, delta_cell] = PM_ArtRep_find_artifact_timepoints(subpar)
...
for k = 1:length(par.scans_to_include)
    ...
    M = []; P = [];
    for i = 1:num_sess
        P{i} = par.rascanfilesByRun{k}; %d.imagefilenames{subj}{i} %spm_select(Inf,'image',['Select data images for session'  num2str(i) ':']);
        if realignfile == 1
            
            %scan_dir = fullfile(par.niiDir, ['scan01']);
            %mvmt_file = dir([scan_dir, '/rp*']);
            %Mh = load(fullfile(scan_dir, mvmt_file.name));
            %startIdx = sum(par.numvols(1:k)) - par.numvols(1) + 1;
            %endIdx = sum(par.numvols(1:k));
            %M{i} = Mh(startIdx:endIdx,:);
            
            Mh = load(fullfile(par.subdir, 'figures2','record.mat'));
            rawMParams_xyz_h = Mh.mparams{k}(:,1:3);
            rawMParams_xyz = rawMParams_xyz_h - repmat(rawMParams_xyz_h(1,:),size(rawMParams_xyz_h,1),1);
            
            rawMParams_rot = Mh.mparams{k}(:,4:6);
            
            %remove leading row
            M{i} = [rawMParams_xyz(2:end,:) rawMParams_rot(2:end,:)];
