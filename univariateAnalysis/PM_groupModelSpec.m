function PM_groupModelSpec(subpar)
...
for t = 1:length(par.tasks)
    ...
    if strcmp(par.stat, 'pt')
        yTemplate1 = load(par.task{t}.conTemplate1);
        yTemplate2 = load(par.task{t}.conTemplate2);
        
        yCons1 = yTemplate1.SPM.xCon;
        yCons2 = yTemplate2.SPM.xCon;
        
        for c= 1:length(yCons1);
                        
            %matlabbatch{1}.spm.stats.factorial_design.des.pt.scans = [];
            
            if ~exist(fullfile(par.expt_dir, 'group_analyses', par.tasks{t}, par.task{t}.SPMcons1(c).name));
                mkdir(fullfile(par.expt_dir, 'group_analyses', par.tasks{t}, par.task{t}.SPMcons1(c).name));
            end
            
            %jobs{c}.stats{1}.factorial_design.dir = {fullfile(expt_dir, 'group_analyses', tasks{t}, yCons(c).name)};
            matlabbatch{1}.spm.stats.factorial_design.dir = {fullfile(par.expt_dir, 'group_analyses', par.tasks{t}, par.task{t}.SPMcons1(c).name)};
            
            matlabbatch{1}.spm.stats.factorial_design.masking.em{1} = par.exMask;
            
            matlabbatch{1}.spm.stats.factorial_design.des.pt.pair = [];
            for s = 1:length(par.task{t}.cons{c}.scans)
               
               	matlabbatch{1}.spm.stats.factorial_design.des.pt.pair(s).scans{1,1} = par.task{t}.cons{c}.scans{s,1};
                matlabbatch{1}.spm.stats.factorial_design.des.pt.pair(s).scans{2,1} = par.task{t}.cons{c}.scans{s,2};

                %jobs{c}.stats{1}.factorial_design.des.t1.scans{s} = par.task{t}.cons{c}.scans{s};
            end
        ...
        for c= 1:length(yCons);
            ...
            jobs{c}.stats{1}.factorial_design.dir = par.task{t}.cons{c}.dir;
             
            if ~exist(jobs{c}.stats{1}.factorial_design.dir{1});
                mkdir(jobs{c}.stats{1}.factorial_design.dir{1});
            end
            ...
            if ~isempty(par.covVec)
                jobs{c}.stats{1}.factorial_design.cov(1).cname = par.covName{c};
                jobs{c}.stats{1}.factorial_design.cov(1).c = par.covVec{c};
