%  Adapted from Jimmy Shen's rri_orient.

%  Given a .nii file, a new file name, and an orientation transform vector, writes out a
%  transformed version of the nii file.

%  If no transform vector is given, uses the vector [5 1 6], which will
%  transform an image written out by kendrick's preprocessing stream into
%  standard orientation 

function [nii, orient, pattern] = PM_reorientNifti(niiFile, newFile, transformVec)
    ...
    if ~exist('transformVec')
    transformVec = [5 4 6]; %Verified by AG, 04/12/11
    end
